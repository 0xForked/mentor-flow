...
export const AvailabilityDaySection = (v: { day: AvailabilityDay }) => {
  ...
  const saveChanges = useMutation(updateAvailability, {
    onSuccess: (resp) => {
      ...
      setState(GlobalStateKey.UpdateAvailabilityTimeData, false);
      setLoadingItem(null);
    },
    onError: (error) => {
      setState(GlobalStateKey.UpdateAvailabilityTimeData, false);
        ...
        <div className="flex h-[36px] items-center gap-4 md:w-32">
          {loadingItem === `switch_${v.day.id}` ? (
            ...
            <Switch
              ...
              disabled={states[GlobalStateKey.UpdateAvailabilityTimeData]}
            />
          )}
            ...
            <div className="flex flex-col gap-2">
              <div className="flex gap-1 last:mb-0 sm:gap-2">
                ...
                <Button
                  ...
                  disabled={saveChanges.isLoading || states[GlobalStateKey.UpdateAvailabilityTimeData]}
                >
              ...
              {v.day?.extend_times?.map((time, index) => (
                <div className="flex gap-1 last:mb-0 sm:gap-2" key={index}>
                  ...
                  <Button
                    ...
                    disabled={loadingItem === time.id || states[GlobalStateKey.UpdateAvailabilityTimeData]}
                  >
                    {loadingItem === time.id ? (
                      <Loader2 className="w-4 animate-spin" />
                    ) : (
